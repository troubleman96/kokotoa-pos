        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
            ...
            <Card className="card-kokotoa lg:col-span-3 overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {language === 'sw' ? 'Mauzo ya wiki iliyopita' : 'Last 7 days of revenue'}
                  </CardDescription>
                </div>
            ...
            <Card className="card-kokotoa overflow-hidden">
              <CardHeader>
                <CardTitle>{language === 'sw' ? 'Hali ya Leo' : "Today's Pulse"}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">{language === 'sw' ? 'Mauzo' : 'Sales'}</span>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">{language === 'sw' ? 'Miamala' : 'Transactions'}</span>
                    <span className="font-bold text-foreground">{dashboardData?.today.transactions || 0}</span>
                  </div>
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-foreground"
                      style={{ width: `${Math.min(100, (dashboardData?.today.transactions || 0) / 20 * 100)}%` }}
                    />
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-muted/30 border border-border mt-4">
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    {language === 'sw'
                      ? 'Utendaji wako wa leo unaongezeka kwa kasi. Endelea kutoa huduma bora!'
                      : 'Your business activity today is trending upwards. Keep up the great work!'}
                  </p>
                </div>
              </CardContent>
            </Card>
